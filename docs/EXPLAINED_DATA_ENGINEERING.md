...
### Surrogate Keys (Used in BOTH Star and Snowflake Schemas)

**Common misconception**: surrogate keys belong to snowflake schema. In reality, **both** star and snowflake schemas use them — just for slightly different reasons.

**What is a surrogate key?** A system-generated key (integer, sequence, or hash) that replaces the natural/business key as the primary key in dimension tables.

```
Natural key:   device_id = "DEV_001"     (from source system)
Surrogate key: device_key = 1            (generated by your warehouse)
```

**Why Star Schema needs surrogate keys — SCD Type 2:**

This is the #1 reason. When a device updates its firmware, you keep both versions:
...
`DEV_001` appears twice — you can't use it as PK. The surrogate key gives each historical version a unique row. Our `dim_devices` does exactly this.

**Why Snowflake Schema needs surrogate keys — normalized joins:**

In addition to SCD Type 2, snowflake schema normalizes dimensions into sub-tables. Each needs its own surrogate key for joining:

```sql
-- Star Schema: 1 JOIN on business key (readable)
SELECT d.cluster_id, AVG(f.avg_temperature)
FROM fct_hourly_readings f
JOIN dim_devices d ON f.device_id = d.device_id    -- business key works fine

-- Snowflake Schema: 2 JOINs through surrogate keys
SELECT c.cluster_city, AVG(f.avg_temperature)
FROM fct_readings f
JOIN dim_device d ON f.device_sk = d.device_sk     -- surrogate key
JOIN dim_cluster c ON d.cluster_sk = c.cluster_sk  -- another surrogate key
```

**Other reasons for surrogate keys (both schemas):**
- **Source system changes**: if source renumbers "DEV_001" to "DEVICE-001", surrogate key stays the same
- **Multi-source integration**: two systems both have "device 001"? Surrogate keys prevent collisions
- **Performance**: integer joins are faster than string joins on large tables
- **Null handling**: natural keys can't be NULL, but some source data has no key — surrogate key fills the gap

**How they connect in star schema:**
```
fct_hourly_readings
├── device_key (FK) ──→ dim_devices.device_key (surrogate)
├── date_key (FK)   ──→ dim_time.date_key (surrogate)
├── avg_temperature
├── avg_humidity
└── reading_count
```

The fact table stores only integer foreign keys — small, fast, scannable.

**Interview tip:** If asked "why surrogate keys?", lead with SCD Type 2 — it's the most concrete, undeniable reason. Then mention source independence and performance.

### Time Dimension (dim_time) — Used in BOTH Star and Snowflake Schemas

Another common misconception: dim_time is a snowflake schema concept. It's actually one of the most universal dimension tables, used in virtually every star schema.

The most useful piece borrowed from Snowflake Schema into Star Schema:

**What is dim_time?** A pre-built lookup table with one row per time grain (per day, per hour, or per minute). Instead of extracting date parts in every query, you join to it.

```sql
-- Full dim_time example (date grain — one row per day)
CREATE TABLE dim_time (
    date_key        INT PRIMARY KEY,    -- 20260402 (surrogate key)
    full_date       DATE,               -- 2026-04-02
    day_of_week     VARCHAR,            -- Wednesday
    day_of_month    INT,                -- 2
    week_of_year    INT,                -- 14
    month_num       INT,                -- 4
    ...
    is_holiday      BOOLEAN,            -- FALSE (US holidays, custom)
    is_business_day BOOLEAN,            -- TRUE
    fiscal_year     INT,                -- 2026 (company-specific)
    fiscal_quarter  INT,                -- 2
    season          VARCHAR             -- Spring
);

-- Pre-computed time attributes (computed once, queried millions of times)
SELECT * FROM dim_time WHERE date_key = 20260330;
-- Returns: is_weekend=false, is_business_day=true, fiscal_quarter=Q1,
--          day_name=Monday, season=Spring, ...
```

**Why not just use DATE_PART() every time?**
- **Business logic centralized**: holidays, fiscal year, pay periods defined once — not scattered across 50 queries
- **BI tool requirement**: Tableau, Looker, Power BI expect a date dimension to build drill-down hierarchies (Year → Quarter → Month → Week → Day)
- **Custom attributes impossible with DATE_PART()**: `is_black_friday`, `is_company_holiday`, `sprint_number`, `pay_period` can't be computed from a date function
- **Performance**: pre-computed vs computed-per-row on billions of rows

**In our pipeline:** `int_readings_enriched` computes `hour_of_day`, `day_of_week`, `is_weekend`, `is_business_hours` inline on the fact table. A proper dim_time would centralize these into a reusable dimension. This is fine for our scale, but in production you'd have a shared dim_time.

**The dbt_date package** (already in our project) can generate dim_time automatically:
```sql
-- dbt_date provides get_date_dimension() to generate a complete date spine
SELECT * FROM {{ dbt_date.get_date_dimension("2024-01-01", "2026-12-31") }}
```
...
ACID is a set of 4 guarantees that make database operations reliable. Without them, data gets corrupted, lost, or inconsistent.
...
```sql
-- Our COPY INTO loads 5 files into sensor_readings.
-- If file #3 has a parse error with ON_ERROR = 'ABORT_STATEMENT':
--   Without atomicity: files 1-2 loaded, 3-5 not → partial data, broken pipeline
--   With atomicity:    entire COPY INTO rolls back → 0 files loaded, clean state
...
-- With atomicity: entire transaction rolls back, both balances unchanged
```

**C — Consistency: Data always moves from one valid state to another.**

The database enforces all rules (constraints, types, foreign keys) before and after every transaction. You can never end up in an "impossible" state.

```sql
-- Snowflake enforces NOT NULL on our raw_data column:
INSERT INTO sensor_readings (raw_data) VALUES (NULL);
-- ERROR: NULL not allowed → transaction rejected, table stays consistent

-- Without consistency: NULLs creep in, downstream dbt models break silently
```

**I — Isolation: Concurrent transactions don't interfere with each other.**

Two users/processes running queries at the same time each see a consistent snapshot. One doesn't see the other's half-finished work.

```sql
-- Airflow runs COPY INTO at 14:00 (loading new data)
-- An analyst runs SELECT COUNT(*) at 14:00 (reading data)
--
-- Without isolation: analyst might count some but not all new rows (dirty read)
-- With isolation: analyst sees either ALL new rows or NONE — a clean snapshot
--
-- Snowflake uses MVCC (Multi-Version Concurrency Control):
--   each query sees a snapshot of data at the moment it started
--   writers don't block readers, readers don't block writers
```

**D — Durability: Once committed, data survives crashes.**

After a `COMMIT` succeeds, the data is persisted permanently — even if the server crashes 1 second later.

```sql
-- Our Lambda writes data, COPY INTO loads it, Snowflake commits.
-- If Snowflake's server crashes right after commit:
--   Without durability: data gone, need to re-run COPY INTO
--   With durability: data is safe on disk, available after restart
```

**Where ACID exists and doesn't exist:**

| System | ACID? | Why |
|--------|-------|-----|
| **Snowflake** | Yes | Full ACID on all operations |
| **PostgreSQL** (our Airflow metadata DB) | Yes | Full ACID |
| **Plain S3 files** | No | If a write fails halfway, you get a corrupt file. No rollback. |
| **S3 + Iceberg** | Yes | Iceberg adds ACID on top of S3 via atomic metadata commits |
| **S3 + Delta Lake** | Yes | Delta adds ACID via transaction log |
| **Kafka** | Partial | Atomic writes within a partition, but no cross-partition transactions |

**Why this matters for our pipeline:**
- `COPY INTO` is atomic — if something fails, no partial data loaded
- Snowflake isolation means our Airflow DAG and analysts can query simultaneously without conflicts
- Plain S3 has no ACID — that's why our Iceberg tables (mart layer) add it on top of S3, enabling time travel and atomic updates

**Interview tip:** Don't just recite "Atomicity, Consistency, Isolation, Durability." Give a concrete example: "In our pipeline, COPY INTO is atomic — if one of 5 files has a parse error, the entire load rolls back. Without atomicity, we'd have partial data that silently breaks downstream aggregations."
...
# Part 10: Operational Lessons Learned

## Airflow on Small EC2 Instances (t4g.small, 2GB RAM)

### The Problem
Running Airflow with **Cosmos (DbtTaskGroup)** on a 2GB instance causes the scheduler's DAG file processor to get OOM-killed. Every 2 minutes, the scheduler imports the DAG file → imports Cosmos → imports dbt-core + all dbt packages (dbt_expectations, dbt_date, dbt_utils). That single import eats ~1GB. With Postgres + webserver + scheduler already running, it exceeds available memory.

### Symptoms
```
WARNING - Killing DAGFileProcessorProcess (PID=235)
INFO - DAG iot_pipeline is missing and will be deactivated.
INFO - Deleted DAG iot_pipeline in serialized_dag table
```

The scheduler kills the file processor after timeout, then thinks the DAG no longer exists and deactivates it.

### Tuning Attempts That Helped (But Didn't Fully Solve)
```yaml
# docker-compose.yml environment variables
AIRFLOW__SCHEDULER__MIN_FILE_PROCESS_INTERVAL: "120"     # Parse DAGs less often
AIRFLOW__SCHEDULER__DAG_DIR_LIST_INTERVAL: "120"          # Scan DAG folder less often
AIRFLOW__SCHEDULER__PARSING_PROCESSES: "1"                # Only 1 parser process
AIRFLOW__CORE__DAGBAG_IMPORT_TIMEOUT: "180"               # Allow longer imports
AIRFLOW__SCHEDULER__DAG_FILE_PROCESSOR_TIMEOUT: "180"     # Don't kill parser too early
AIRFLOW__CORE__PARALLELISM: "4"                           # Limit concurrent tasks
AIRFLOW__CORE__MAX_ACTIVE_TASKS_PER_DAG: "4"              # Limit per-DAG tasks
```

### Cosmos Load Modes
```python
from cosmos import LoadMode, RenderConfig

# AUTOMATIC (default) — runs `dbt ls` which loads ALL dbt packages. HEAVY.
RenderConfig(load_method=LoadMode.AUTOMATIC)

# CUSTOM — Python-based file parsing, does NOT run dbt. LIGHTER.
RenderConfig(load_method=LoadMode.CUSTOM)

# DBT_MANIFEST — reads pre-built manifest.json. LIGHTEST.
# Requires running `dbt compile` first to generate the manifest.
RenderConfig(load_method=LoadMode.DBT_MANIFEST)
```

### The Real Fix
**Upgrade to t4g.medium (4GB RAM, ~$12/month vs ~$6/month)** or run Airflow locally where memory isn't constrained. On 2GB, Cosmos + dbt-core + Airflow + Postgres simply doesn't fit.

### Interview Takeaway
"We ran into OOM issues running Airflow with Cosmos on a 2GB instance. The DAG file processor kept getting killed because each DAG parse imported the full dbt-core library. We tuned parsing intervals, reduced parallelism, and switched to Cosmos CUSTOM load mode, but ultimately needed more RAM. This taught me that Cosmos/dbt-integrated DAGs have a higher memory baseline than standard Airflow DAGs."
...
### The Problem
Snowflake accounts with MFA/TOTP enabled reject password-only connections:
```
DatabaseError: 250001 (08001): Failed to authenticate: MFA with TOTP is required.
```

This breaks automated connections from Airflow, dbt, and scripts.

### The Solution: RSA Key-Pair Authentication
Instead of password, authenticate with an RSA private key. No MFA prompt needed.
...
```sql
-- Extract the key body (no BEGIN/END headers)
ALTER USER my_user SET RSA_PUBLIC_KEY='MIIBIjANBgk...entire key...AQAB';
```

**Step 3: Use in Python (snowflake-connector-python)**
```python
...
conn = snowflake.connector.connect(
    account="...",
    user="...",
    private_key=private_key_bytes,  # instead of password
    role="IOT_TRANSFORMER",
    warehouse="IOT_WH",
)
```

**Step 4: Use in dbt (profiles.yml)**
```yaml
iot_pipeline:
  target: dev
  outputs:
...
**Step 5: Mount key in Docker**
```yaml
# docker-compose.yml volumes
volumes:
  - ~/.ssh/snowflake/rsa_key.p8:/opt/airflow/keys/rsa_key.p8:ro
```

### Security Notes
- Private key (`rsa_key.p8`) should NEVER be committed to git — add to `.gitignore`
- Use `:ro` (read-only) mount in Docker
- For production: store in AWS Secrets Manager or HashiCorp Vault
- Rotate keys periodically: generate new pair, `ALTER USER SET RSA_PUBLIC_KEY_2`, then swap

### Interview Takeaway
"Our Snowflake account had MFA enabled, which broke automated connections from Airflow and dbt. We switched to RSA key-pair authentication — generate an RSA key pair, assign the public key to the Snowflake user, and authenticate with the private key. This bypasses MFA while maintaining security, and is the recommended approach for service accounts in production."

---

## EventBridge + Lambda (Decoupled Producer Pattern)

### What We Set Up
```
EventBridge Rule (hourly cron) → Lambda (generate data) → S3 (raw JSON)
                                                              ↓
Airflow DAG (hourly schedule) → COPY INTO Snowflake → dbt transforms
```

Producer (Lambda) and consumer (Airflow) run **independently**. Neither knows about the other.

### Key Commands
```bash
# Create rule — trigger Lambda every hour
aws events put-rule --name iot-fleet-hourly-generate \
    --schedule-expression "rate(1 hour)" --state ENABLED

# Add Lambda as target
aws events put-targets --rule iot-fleet-hourly-generate \
    --targets '[{"Id": "lambda-target", "Arn": "arn:aws:lambda:..."}]'

# Disable (stop generating data)
aws events disable-rule --name iot-fleet-hourly-generate

# Re-enable
aws events enable-rule --name iot-fleet-hourly-generate
...
### Why Decouple?
| Coupled (Airflow triggers Lambda) | Decoupled (EventBridge triggers Lambda) |
|---|---|
| If Airflow is down, no data generated | Lambda keeps generating regardless |
| Airflow controls Lambda schedule | Each runs on its own schedule |
| Single point of failure | Independent failure domains |
| Hard to test Lambda in isolation | Lambda works standalone |

### How Recovery Works
Snowflake `COPY INTO` is **idempotent** — it tracks which S3 files have already been loaded. If Airflow was down for 6 hours:
1. Lambda kept writing to S3 (6 new files)
2. Airflow comes back, runs `COPY INTO`
3. Snowflake loads all 6 files in one go
4. No data loss, no duplicates, no backfill needed
