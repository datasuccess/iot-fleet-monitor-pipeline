"""
Data Quality Dashboard — Streamlit in Snowflake
"""
...
quality = run_query("""
    SELECT batch_ts, source_file, total_records, null_count, null_pct,
           duplicate_count, duplicate_pct, quarantined_count,
           late_arrival_count, quality_score
...
if not quality.empty:
    ...
    with col1:
        fig = px.bar(
            quality.sort_values("batch_ts"),
            x="batch_ts", y=["null_pct", "duplicate_pct"],
            title="Null % vs Duplicate % per Batch", barmode="group",
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        fig = px.bar(
            quality.sort_values("batch_ts"),
            x="batch_ts", y=["quarantined_count", "late_arrival_count"],
            title="Quarantined & Late Arrivals per Batch", barmode="group",
...
quarantine = run_query("""
    SELECT device_id, reading_ts, temperature, humidity, pressure,
           battery_pct, rejection_reasons, loaded_at
...
late = run_query("""
    SELECT device_id, reading_ts, loaded_at, arrival_delay_minutes
...
if not late.empty:
    ...
    fig = px.histogram(
        ...
        title="Arrival Delay Distribution (minutes)", nbins=30,
    )
    st.plotly_chart(fig, use_container_width=True)
else:
